...
describes what users get. Pick a format with `-o`, or let bibleapi choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
bibleapi <command> -o table   # aligned columns for reading
bibleapi <command> -o jsonl   # one JSON object per line, for piping
bibleapi <command> -o json    # a single JSON array
bibleapi <command> -o csv     # spreadsheet friendly
bibleapi <command> -o tsv     # tab-separated
bibleapi <command> -o url     # just the URL column
bibleapi <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
bibleapi <command> --fields id,title,url
```
...
```bash
bibleapi <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
bibleapi <command>            # a table, because this is a terminal
bibleapi <command> | wc -l    # JSONL, because this is a pipe
```
